...
function openCity_stickerInteractive(evt, cityName) {
  ...
  tabcontentOne = document.getElementsByClassName(
    "tabcontent_stickerInteractive"
  );
  ...
  $("div.changeable_code").click(function (e) {
    ...
    clone.click(function () {
      if (oddClick) {
        ...
        console.log(
          $(red_x_div).parent().attr("id") == $(clone.parent()).attr("id")
        );
...
$("#S1").on("click", function () {
  ...
  $("#S1SVG").css({ display: "block" });
  $("#S2SVG").css({ display: "none" });
  $("#S3SVG").css({ display: "none" });
  $("#S4SVG").css({ display: "none" });
  $("#S5SVG").css({ display: "none" });
  $("#S6SVG").css({ display: "none" });
  $("#S7SVG").css({ display: "none" });
  $("#S8SVG").css({ display: "none" });
  $("#S9SVG").css({ display: "none" });
  $("#S10SVG").css({ display: "none" });
});
$("#S2").on("click", function () {
  $("#Background").css({ fill: "#ffeadb" });
  $("#S1SVG").css({ display: "none" });
  $("#S2SVG").css({ display: "block" });
  $("#S3SVG").css({ display: "none" });
  $("#S4SVG").css({ display: "none" });
  $("#S5SVG").css({ display: "none" });
  $("#S6SVG").css({ display: "none" });
...
var oldVal = "";
$("#defaulttext").on("change keyup paste", function () {
...
document.getElementById("Save").addEventListener("click", function () {
  const handArtElement = document.getElementById("handwrapper");

  if (!handArtElement) {
    alert("Hand art not found!");
    return;
  }

  html2canvas(handArtElement)
    .then((canvas) => {
      const link = document.createElement("a");
      link.href = canvas.toDataURL("image/png");
      link.download = "hand_art.png";
      link.click();
    })
    .catch((error) => {
